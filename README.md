...
- `metrics.csv`：基线规则方案与优化方案的覆盖率、布通率、面积占比、线长、违规数和运行时间。
- `layout.svg`：金手指、障碍区、测试点和布线路径可视化。

## 算法说明

布点阶段先根据尾板尺寸、网格步长和障碍区生成候选测试点；再用金手指横向分布近似 K-means 分组，得到测试点初始目标带；随后用局部禁忌搜索降低线长和拥挤度。新增的 `pso_hanan_tabu_astar` 参考 `secret4233/auto_routing` 中 PSO/Hanan 点优化思想，但按本工程数据结构重新实现，不直接复制外部源码；它用粒子群搜索布点参数，再通过禁忌搜索和 A* 网格布线输出最终方案。实验中的三组方法分别为：
